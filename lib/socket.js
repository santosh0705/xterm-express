...
var socket = (server, options) => {
  ...
  io.on('connection', socket => {
    ...
    new Promise( (resolve, reject) => {
      if (askusername) {
        var buf = ''
        var timer = setTimeout( () => reject('Timed out due to inactivity.'), 60000)

        socket.emit('data', 'Enter username: ')
        socket.on('input', data => {
          clearTimeout(timer)
          timer = setTimeout( () => reject('Timed out due to inactivity.'), 60000)

          switch (data) {
            case '\x03':
              socket.emit('data', '^C')
              clearTimeout(timer)
              reject('Terminated by user.')
              break;
            case '\x7f':
            case '\b':
              if (buf.length > 0) {
    ...
    }, data => socket.emit('exited', data)) // promise reject
  })
  return io
}
